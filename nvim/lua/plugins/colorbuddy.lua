-- Function to apply colorbuddy theme (can be called multiple times)
local function apply_colorbuddy_theme()
	-- Reload nixcolors to get fresh colors based on vim.o.background
	...
	Group.new('@function',                           c.BrightBlue,         c.none, no)
	Group.new('@function.builtin',                   c.BrightBlue,         c.none, no)
...
-- Make the function globally accessible for theme reloading
_G.reload_colorbuddy_theme = apply_colorbuddy_theme
...
return {
	{
		...
		config = function()
			-- To load other modules from the flake's ./nvim folder
			vim.opt.rtp:prepend(luamodpath)

			-- Apply theme on initial load
			apply_colorbuddy_theme()
		end,
	},
}
